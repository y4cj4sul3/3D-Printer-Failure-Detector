...
def segments_to_meshdata(segments):  # edges only on extrusion
    ...
    verts = []
    edges = []
    time_height = []  # time level up and height after level up
    del_offset = 0  # to travel segs in a row, one gets deleted, need to keep track of index for edges
    for i in range(len(segs)):
        if i >= len(segs)-1:
            time_height.append([len(edges), segs[i].coords['Z']])
        ...
        # elif segs[i].style == 'extrude' and segs[i+1].style == 'travel':
            # pass
...
class GcodeParser:
    # global, to access in other classes(to access RGB values in comment above when parsing M163). Theres probably better way
    comment = ""
    ...
    def error(self, msg):
        print("[ERROR] Line %d: %s (Text:'%s')" %
              (self.lineNb, msg, self.line))
        raise Exception("[ERROR] Line %d: %s (Text:'%s')" %
                        (self.lineNb, msg, self.line))
...
class GcodeModel:
    ...
    def do_G1(self, args, type):
        ...
        # only add seg if XYZ changes (skips "G1 Fxxx" only lines and avoids double vertices inside Blender, because XYZ stays the same on such a segment.
    ...
    def do_M163(self, args):
        ...
        weight = args['P']
        # change CMYKW
        # +3 weil ersten 3 stellen RGB sind, need only CMYKW values for extrude
        col[extr_idx+3] = weight
        self.color = col
        ...
        for seg in self.segments:
            ...
            if seg.distance > subd_threshold:
                ...
                for i in range(len(interp_coords)):
                    ...
                    new_coords = {"X": interp_coords[i][0], "Y": interp_coords[i]
                                  [1], "Z": interp_coords[i][2], "F": seg.coords["F"]}
                    ...
                    # write segment only if movement changes, avoid double coordinates due to same start and endpoint of linspace
                    if new_coords['X'] != coords['X'] or new_coords['Y'] != coords['Y'] or new_coords['Z'] != coords['Z']:
                ...
                #print("dist < threshold coordinate", seg.coords)
    ...
    def draw(self, split_layers=False):
        verts, edges, time_height = segments_to_meshdata(self.segments)
        ...
        #         if len(verts) > 0:
        #             obj_from_pydata(str(i), verts, edges,
        #                             close=False, collection_name="Layers")
        #             i += 1
        ...
        # # vertex_dict to txt file
        #     f = open(bpy.path.abspath("//")+"vertex_data.txt", "w")
        #     tempstring_vertex_data = []
        #     for i, seg in enumerate(self.segments):
        #         x, y, z = seg.coords['X'], seg.coords['Y'], seg.coords['Z']
        #         # store vertex_data in txt file, one dict per line with meta data
        #         #vertex_data={"style":seg.style, "color":None, "multiplier":None, "feed":None }
        #         vertex_data = {"coords": seg.coords, "style": seg.style, "toolnumber": seg.toolnumber, "color": seg.color,
        #                        "multiplier": None, "feed": seg.coords["F"], "layer": seg.layerIdx}  # DEBUGGING with coords
        #         tempstring_vertex_data.append(str(vertex_data))

        #     f.write("\n".join(tempstring_vertex_data))
        #     f.close()
